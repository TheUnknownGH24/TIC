Public Class frm8SI_PAYMENTREQUEST_PO
    ...
    Sub PO_UPDATE()
        ...
        If txtLESSAMOUNT.Text = 0 Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_purchasedorder set status=@status where id=@id", cn)
    ...
    Sub DisplayNO()
        ...
        cn.Open()
        adp = New MySqlDataAdapter("select * from tbl_8si_paymentrequest where transaction_no = '" & txtPONO.Text & "' ORDER BY id DESC LIMIT 1", cn)
        dt = New DataTable
        adp.Fill(dt)
    ...
    Sub LoadPOList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_purchasedorder where `MANAGER_APPROVAL` = 'APPROVED' AND `GM_APPROVAL` = 'FOR APPROVAL' AND `COO_APPROVAL` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_purchasedorder where `po_no` like '%" & TextBox1.Text & "%' AND `MANAGER_APPROVAL` = 'APPROVED' AND `GM_APPROVAL` = 'FOR APPROVAL' AND `COO_APPROVAL` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadItemList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_purchasedorderlist where `po_no` like '" & txtPAYMENTTYPE.Text & "' and supplier_name like '" & txtSUPPLIER_SELECT.Text & "'", cn)
        dr = cm.ExecuteReader
        While dr.Read
            i += 1
