Public Class frm8SI_PAYMENTREQUEST_GM
    ...
    Sub LoadList()
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_paymentrequest where `manager_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `gm_approval` = 'FOR APPROVAL' AND `coo_approval` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE' ", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_paymentrequest where controlno like '%" & TextBox1.Text & "%' AND `manager_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `gm_approval` = 'FOR APPROVAL' AND `coo_approval` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE'", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub btnAPPROVED_Click(sender As Object, e As EventArgs) Handles btnAPPROVED.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_paymentrequest set gm_approval=@gm_approval, gm_comment=@gm_comment, status=@status where id=@id", cn)
    ...
    Private Sub btnRETURN_Click(sender As Object, e As EventArgs) Handles btnRETURN.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_paymentrequest set manager_approval=@manager_approval, finance_approval=@finance_approval, gm_approval=@gm_approval, gm_comment=@gm_comment, status=@status where id=@id", cn)
    ...
    Private Sub frm8SI_PAYMENTREQUEST_GM_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
