Public Class frmTIC_RF_REPLENISHMENT
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("INSERT into tbl_tic_rf_replenishment (reference, date, employee, department, depcode, purpose, revolving_fund, total_liquidation, unreplenish, endorsed_approval, endorsed_comment, finance_approval, finance_comment, final_approval, final_comment, date_time, status) values(@reference, @date, @employee, @department, @depcode, @purpose, @revolving_fund, @total_liquidation, @unreplenish, @endorsed_approval, @endorsed_comment, @finance_approval, @finance_comment, @final_approval, @final_comment, @date_time, @status)", cn)
            With cm.Parameters
    ...
    Sub DisplayCANO()
        ...
        cn.Open()
        adp = New MySqlDataAdapter("select * from tbl_tic_rf_replenishment where `reference` = '" & txtREFERENCE.Text & "'", cn)
        dt = New DataTable
        adp.Fill(dt)
    ...
    Private Sub frmTIC_RF_REPLENISHMENT_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
