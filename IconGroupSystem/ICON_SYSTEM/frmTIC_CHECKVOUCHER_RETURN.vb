Public Class frmTIC_CHECKVOUCHER_RETURN
    ...
    Sub LoadCVList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_checkvoucher where `STATUS` = 'RETURNED' ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_checkvoucher set payee_name=@payee_name, reference_code=@reference_code, subreference_code=@subreference_code, checkvoucher_date=@checkvoucher_date, check_no=@check_no, bank_branch=@bank_branch, check_date=@check_date, particulars=@particulars, debit_total=@debit_total, credit_total=@credit_total, date_time=@date_time, status=@status where id=@id", cn)
    ...
    Sub DeleteInCVList()
        ...
        cm = New MySqlCommand("DELETE FROM tbl_tic_checkvoucher_accounttitle WHERE `cv_no` like '" & txtCONTROL.Text & "'", cn)
    ...
    Sub SaveInCVList()
        ...
        MsgBox("Check Voucher has been Successfully Updated!", vbInformation + vbOKOnly, "CONFIRMATION")
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_checkvoucher where `cv_no` like '%" & TextBox3.Text & "%' AND `STATUS` = 'RETURNED'", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub frmTIC_CHECKVOUCHER_RETURN_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
