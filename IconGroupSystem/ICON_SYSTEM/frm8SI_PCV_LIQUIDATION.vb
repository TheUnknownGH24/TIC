Public Class frm8SI_PCV_LIQUIDATION
    ...
    Sub LoadPCVList()
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_pettycash where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'RELEASED' AND `STATUS` = 'ACTIVE' ", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_pettycash where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'RELEASED' AND `STATUS` = 'ACTIVE' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%' ", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("INSERT into tbl_8si_pettycash_liquidation (pcv_no, reference, date, employee, department, depcode, purpose, total_caamount, total_liquidation, total_balance, endorsed_approval, endorsed_comment, finance_approval, finance_comment, final_approval, final_comment, date_time, status) values(@pcv_no, @reference, @date, @employee, @department, @depcode, @purpose, @total_caamount, @total_liquidation, @total_balance, @endorsed_approval, @endorsed_comment, @finance_approval, @finance_comment, @final_approval, @final_comment, @date_time, @status)", cn)
            With cm.Parameters
    ...
    Sub PCV_UPDATE()
        ...
        cn.Open()
        cm = New MySqlCommand("Update tbl_8si_pettycash set status=@status where id=@id", cn)
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_pettycash where pcv_no like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'RELEASED' AND `STATUS` = 'ACTIVE'", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_pettycash where pcv_no like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'RELEASED' AND `STATUS` = 'ACTIVE' and `department` like '%" & txtDEPARTMENTDATA.Text & "%'", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub frm8SI_PCV_LIQUIDATION_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
