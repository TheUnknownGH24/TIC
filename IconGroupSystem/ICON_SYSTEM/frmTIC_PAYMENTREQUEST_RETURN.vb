Public Class frmTIC_PAYMENTREQUEST_RETURN
    ...
    Sub LoadList()
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_paymentrequest where `STATUS` = 'RETURNED' ", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_paymentrequest where `STATUS` = 'RETURNED' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%' ", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_paymentrequest where controlno like '%" & TextBox1.Text & "%' AND `STATUS` = 'RETURNED'", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_paymentrequest where controlno like '%" & TextBox1.Text & "%' AND `STATUS` = 'RETURNED' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%'", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_paymentrequest set type_payment=@type_payment, date_needed=@date_needed, mode_payment=@mode_payment, date=@date, mode_release=@mode_release, account_name=@account_name, account_no=@account_no, bank_name=@bank_name, amount=@amount, amount_words=@amount_words, payment_remarks=@payment_remarks, reference_code=@reference_code, sub_referencecode=@sub_referencecode, status=@status where id=@id", cn)
