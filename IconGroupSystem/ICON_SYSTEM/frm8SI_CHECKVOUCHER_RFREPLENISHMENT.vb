Public Class frm8SI_CHECKVOUCHER_RFREPLENISHMENT
    ...
    Sub CV_UPDATE()
        ...
        cn.Open()
        cm = New MySqlCommand("Update tbl_8si_rf_replenishment set status=@status where id=@id", cn)
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_rf_replenishment where `rf_no` like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'APPROVED BY MS.VTY'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadRFList()
        ...
        Dim i As Integer
        cn.Open()
        'cm = New MySqlCommand("select * from tbl_8si_rf_replenishment where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'ACTIVE' AND `UNREPLENISH` < 0 ", cn)

        cm = New MySqlCommand("select * from tbl_8si_rf_replenishment where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'APPROVED BY MS.VTY' ", cn)
