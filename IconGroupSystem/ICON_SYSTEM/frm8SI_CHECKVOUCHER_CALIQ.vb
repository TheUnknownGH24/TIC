Public Class frm8SI_CHECKVOUCHER_CALIQ
    ...
    Sub CV_UPDATE()
        ...
        cn.Open()
        cm = New MySqlCommand("Update tbl_8si_cashadvance_liquidation set status=@status where id=@id", cn)
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_cashadvance_liquidation where `cano_liquidation` like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'APPROVED BY MS.VTY' AND `TOTAL_BALANCE` < 0", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadCAList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_cashadvance_liquidation where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'APPROVED BY MS.VTY' AND `TOTAL_BALANCE` < 0 ", cn)
        dr = cm.ExecuteReader
