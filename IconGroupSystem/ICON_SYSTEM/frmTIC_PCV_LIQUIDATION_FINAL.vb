Public Class frmTIC_PCV_LIQUIDATION_FINAL
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_pettycash_liquidation where `pcv_no` like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ON-GOING LIQUIDATION'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadPCVList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_pettycash_liquidation where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ON-GOING LIQUIDATION' ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_pettycash_liquidation set final_approval=@final_approval, final_comment=@final_comment, status=@status where id=@id", cn)
    ...
    Sub PCV_UPDATE()
        ...
        cn.Open()
        cm = New MySqlCommand("Update tbl_tic_pettycash set status=@status where pcv_no=@pcv_no", cn)
    ...
    Private Sub frmTIC_PCV_LIQUIDATION_FINAL_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
