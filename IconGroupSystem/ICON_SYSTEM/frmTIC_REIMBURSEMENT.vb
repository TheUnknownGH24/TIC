Public Class frmTIC_REIMBURSEMENT
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("INSERT into tbl_tic_reimbursement (transaction_no, date, employee, department, depcode, purpose, total_liquidation, endorsed_approval, endorsed_comment, finance_approval, finance_comment, final_approval, final_comment, date_time, status) values(@transaction_no, @date, @employee, @department, @depcode, @purpose,@total_liquidation, @endorsed_approval, @endorsed_comment, @finance_approval, @finance_comment, @final_approval, @final_comment, @date_time, @status)", cn)
            With cm.Parameters
    ...
    Sub DisplayREIMNO()
        ...
        cn.Open()
        adp = New MySqlDataAdapter("select * from tbl_tic_reimbursement where `transaction_no` = '" & txtREFERENCE.Text & "'", cn)
        dt = New DataTable
        adp.Fill(dt)
    ...
    Private Sub frmTIC_REIMBURSEMENT_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
