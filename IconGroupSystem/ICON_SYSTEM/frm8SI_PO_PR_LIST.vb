...
Public Class frm8SI_PO_PR_LIST
    ...
    Private Sub txtSEARCHNAME_TextChanged(sender As Object, e As EventArgs) Handles txtSEARCHNAME.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_purchasedrequest where `controlno` like '%" & txtSEARCHNAME.Text & "%' AND `gm_approval` = 'APPROVED' AND `coo_approval` = 'APPROVED' AND `status` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadREQUESTList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_purchasedrequest where `gm_approval` = 'APPROVED' AND `coo_approval` = 'APPROVED' AND `status` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader
        While dr.Read
            i += 1
            dataGridView1.Rows.Add(i, dr.Item("id").ToString, dr.Item("pr_no").ToString, dr.Item("transaction_no").ToString, dr.Item("employee").ToString, dr.Item("department").ToString, dr.Item("date").ToString, dr.Item("purpose").ToString, dr.Item("date_time").ToString, dr.Item("supplier_1").ToString, dr.Item("supplier_2").ToString, dr.Item("supplier_3").ToString)
    ...
    Private Sub frm8SI_PO_PR_LIST_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
