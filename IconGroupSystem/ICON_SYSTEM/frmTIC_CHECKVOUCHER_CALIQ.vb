Public Class frmTIC_CHECKVOUCHER_CALIQ
    ...
    Sub LoadAccountTitle()
        ...
        cn.Open()
        cm = New MySqlCommand("select * from b_tic_account_title", cn)
        dr = cm.ExecuteReader
        While dr.Read
    ...
    Sub CV_UPDATE()
        ...
        cn.Open()
        cm = New MySqlCommand("Update tbl_tic_cashadvance_liquidation set status=@status where id=@id", cn)
    ...
    Sub DisplayCVNO()
        ...
        cn.Open()
        adp = New MySqlDataAdapter("select * from tbl_tic_checkvoucher where `reference` = '" & txtREFERENCE.Text & "' ORDER BY id DESC LIMIT 1", cn)
        dt = New DataTable
        adp.Fill(dt)
    ...
    Sub SaveInCVList()
        ...
        For Each row As DataGridViewRow In dataGridView1.Rows
            ...
            cm = New MySqlCommand("INSERT into tbl_tic_checkvoucher_accounttitle (cv_no, account_code, account_title, dep_code, debit, credit) values(@cv_no, @account_code, @account_title, @dep_code, @debit, @credit)", cn)
            With cm.Parameters
    ...
    Private Sub PrintDocument1_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        e.Graphics.DrawString("CA_LIQ#", sign, Brushes.Black, 50, 210)
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_cashadvance_liquidation where `cano_liquidation` like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'APPROVED BY MS.VTY' AND `TOTAL_BALANCE` < 0", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadCAList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_cashadvance_liquidation where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'APPROVED BY MS.VTY' AND `TOTAL_BALANCE` < 0 ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub DataGridView2_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView2.CellContentClick
        ...
        If colname = "MANAGER_APPROVAL" Then
            ...
            While dr.Read
                ...
                txtREFERENCECODE.Text = DataGridView2.Rows(e.RowIndex).Cells(2).Value.ToString
                txtSUBREFERENCECODE.Text = DataGridView2.Rows(e.RowIndex).Cells(3).Value.ToString

                txtSUPPLIER.Text = DataGridView2.Rows(e.RowIndex).Cells(5).Value.ToString
                txtPAYEE.Text = DataGridView2.Rows(e.RowIndex).Cells(5).Value.ToString

                txtPARTICULARS.Text = DataGridView2.Rows(e.RowIndex).Cells(8).Value.ToString

                txtAMOUNTDUE.Text = DataGridView2.Rows(e.RowIndex).Cells(11).Value.ToString * (-1)

                txtCOMMENT_1.Text = DataGridView2.Rows(e.RowIndex).Cells(12).Value.ToString
                txtCOMMENT_2.Text = DataGridView2.Rows(e.RowIndex).Cells(13).Value.ToString
                txtCOMMENT_3.Text = DataGridView2.Rows(e.RowIndex).Cells(14).Value.ToString
    ...
    Private Sub frmTIC_CHECKVOUCHER_CALIQ_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
    ...
    Private Sub Button8_Click(sender As Object, e As EventArgs) Handles Button8.Click
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from b_tic_account_title where account_title like '%" & TextBox5.Text & "%'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadDepCode()
        ...
        cn.Open()
        cm = New MySqlCommand("select * from b_tic_account_title where `account_code` like '%" & txtACCTCODE.Text & "%' ", cn)
        dr = cm.ExecuteReader
        While dr.Read
