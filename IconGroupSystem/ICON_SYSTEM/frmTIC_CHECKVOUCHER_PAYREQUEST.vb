...
Public Class frmTIC_CHECKVOUCHER_PAYREQUEST
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_paymentrequest where controlno like '%" & TextBox1.Text & "%' AND `manager_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `gm_approval` = 'APPROVED' AND `coo_approval` = 'APPROVED' AND `STATUS` = 'ACTIVE'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_paymentrequest where `manager_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `gm_approval` = 'APPROVED' AND `coo_approval` = 'APPROVED' AND `STATUS` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader
    ...
    Sub CV_UPDATE()
        ...
        cn.Open()
        cm = New MySqlCommand("Update tbl_tic_paymentrequest set status=@status where id=@id", cn)
    ...
    Private Sub frmTIC_CHECKVOUCHER_PAYREQUEST_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
