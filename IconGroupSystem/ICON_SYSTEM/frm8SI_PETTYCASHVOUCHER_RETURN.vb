Public Class frm8SI_PETTYCASHVOUCHER_RETURN
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_pettycash where `pcv_no` like '%" & TextBox3.Text & "%' AND `STATUS` = 'RETURNED'", cn)
            dr = cm.ExecuteReader
            While dr.Read
                i += 1
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_pettycash where `pcv_no` like '%" & TextBox3.Text & "%' AND `STATUS` = 'RETURNED' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%'", cn)
            dr = cm.ExecuteReader
            While dr.Read
                i += 1
                DataGridView2.Rows.Add(i, dr.Item("id").ToString, dr.Item("pcv_no").ToString, dr.Item("transaction_no").ToString, dr.Item("date").ToString, dr.Item("employee").ToString, dr.Item("department").ToString, dr.Item("depcode").ToString, dr.Item("date_needed").ToString, dr.Item("purpose").ToString, dr.Item("amount").ToString, dr.Item("endorsed_comment").ToString, dr.Item("finance_comment").ToString, dr.Item("final_comment").ToString)
    ...
    Sub LoadPCVList()
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_pettycash where `STATUS` = 'RETURNED' ", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_pettycash where `STATUS` = 'RETURNED' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%' ", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click_1(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_pettycash set purpose=@purpose, amount=@amount, date_time=@date_time, status=@status where id=@id", cn)
    ...
    Sub DeleteInPCVList()
        ...
        cm = New MySqlCommand("DELETE FROM tbl_8si_pettycashlist WHERE `pcv_no` like '" & txtCONTROL.Text & "'", cn)
    ...
    Sub SaveInPCVList()
        ...
        For Each row As DataGridViewRow In dataGridView1.Rows
            ...
            cm = New MySqlCommand("INSERT into tbl_8si_pettycashlist (pcv_no, expenses_categories, amount, remarks) values(@pcv_no, @expenses_categories, @amount, @remarks)", cn)
            With cm.Parameters
    ...
    Private Sub frm8SI_PETTYCASHVOUCHER_RETURN_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
