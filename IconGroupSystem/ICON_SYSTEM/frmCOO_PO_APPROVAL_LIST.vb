...
Public Class frmCOO_PO_APPROVAL_LIST
    ...
    Sub LoadPOList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tblpurchasedorder where `MANAGER_APPROVAL` = 'APPROVED' AND `STATUS` = '' AND `GM_APPROVAL` = 'APPROVED' AND `COO_APPROVAL` = 'FOR APPROVAL' ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub txtSEARCHNAME_TextChanged(sender As Object, e As EventArgs) Handles txtSEARCHNAME.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tblpurchasedorder where `po_no` like '%" & txtSEARCHNAME.Text & "%' AND `MANAGER_APPROVAL` = 'APPROVED' AND `GM_APPROVAL` = 'APPROVED' AND `COO_APPROVAL` = 'FOR APPROVAL'", cn)
        dr = cm.ExecuteReader
