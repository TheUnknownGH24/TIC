Public Class frmTIC_REQUESTSLIP_GM_FORM
    ...
    Sub LoadREQUESTList()
        ...
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_requestsliplist where `controlno` like '%" & txtCONTROL.Text & "%'", cn)
        dr = cm.ExecuteReader
        While dr.Read
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs)
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_requestslip set date_needed=@date_needed, gm_approval=@gm_approval, gm_comment=@gm_comment where id=@id", cn)

            With cm.Parameters
                .AddWithValue("@id", txtID.Text)
                .AddWithValue("@date_needed", txtDATENEEDED.Text)
                .AddWithValue("@gm_approval", "APPROVED")
                .AddWithValue("@gm_comment", txtGMCOMMENT.Text & " - " & txtVALUEDATE.Value)
            End With
            cm.ExecuteNonQuery()
            cn.Close()
            DeleteInREQUESTList()
    ...
    Sub DeleteInREQUESTList()
        ...
        cm = New MySqlCommand("DELETE FROM tbl_tic_requestsliplist WHERE `controlno` like '%" & txtCONTROL.Text & "%'", cn)
    ...
    Private Sub btnPRINT_Click(sender As Object, e As EventArgs) Handles btnPRINT.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_requestslip set date_needed=@date_needed, gm_approval=@gm_approval, gm_comment=@gm_comment, status=@status where id=@id", cn)
    ...
    Private Sub btnRETURN_Click(sender As Object, e As EventArgs) Handles btnRETURN.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_requestslip set MANAGER_APPROVAL=@MANAGER_APPROVAL, GM_APPROVAL=@GM_APPROVAL, gm_comment=@gm_comment, status=@status where id=@id", cn)
            ...
            With cm.Parameters
                ...
                .AddWithValue("@gm_comment", txtCOMMENT.Text)
    ...
    Private Sub txtSEARCHNAME_TextChanged(sender As Object, e As EventArgs) Handles txtSEARCHNAME.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_requestslip where `controlno` like '%" & txtSEARCHNAME.Text & "%' AND `manager_approval` = 'APPROVED' AND `gm_approval` = 'FOR APPROVAL' AND `status` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader

        While dr.Read
            i += 1
            DataGridView2.Rows.Add(i, dr.Item("id").ToString, dr.Item("controlno").ToString, dr.Item("transaction_no").ToString, dr.Item("date_now").ToString, dr.Item("employee_name").ToString, dr.Item("department").ToString, dr.Item("date_needed").ToString, dr.Item("date_time").ToString, dr.Item("manager_comment").ToString)
    ...
    Sub LoadAPPROVALLIST()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_requestslip where `manager_approval` = 'APPROVED' AND `gm_approval` = 'FOR APPROVAL' AND `status` = 'ACTIVE'", cn)
        dr = cm.ExecuteReader
        While dr.Read
            i += 1
            DataGridView2.Rows.Add(i, dr.Item("id").ToString, dr.Item("controlno").ToString, dr.Item("transaction_no").ToString, dr.Item("date_now").ToString, dr.Item("employee_name").ToString, dr.Item("department").ToString, dr.Item("date_needed").ToString, dr.Item("date_time").ToString, dr.Item("manager_comment").ToString)
    ...
    Private Sub DataGridView2_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView2.CellContentClick
        ...
        If colname = "colUPDATE" Then
                cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_requestslip where id like '" & DataGridView2.Rows(e.RowIndex).Cells(1).Value.ToString & "'", cn)
                dr = cm.ExecuteReader
                While dr.Read
                ...
                txtCOMMENT.Text = DataGridView2.Rows(e.RowIndex).Cells(9).Value.ToString
    ...
    Private Sub frmTIC_REQUESTSLIP_GM_FORM_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
