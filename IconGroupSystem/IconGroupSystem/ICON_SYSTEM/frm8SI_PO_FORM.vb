...
Public Class frm8SI_PO_FORM
    ...
    Sub LoadItemList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_purchasedrequestlist where `pr_no` like '" & txtREFERENCE.Text & "' and supplier_name like '" & txtSUPPLIER.Text & "'", cn)
        dr = cm.ExecuteReader
        While dr.Read
            i += 1
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        Else
            ...
            reloadtextPO("Select * from tbl_8si_purchasedorder where pr_no='" & txtREFERENCE.Text & "' AND supplier='" & txtSUPPLIER.Text & "' ")
    ...
    Sub SaveItemList()
        ...
        For Each row_1 As DataGridViewRow In dataGridView1.Rows
            ...
            cm = New MySqlCommand("INSERT into tblwarehouse_po_item (po_no, itemname, qty, status) values(@po_no, @itemname, @qty, @status)", cn)

            With cm.Parameters

                .AddWithValue("@po_no", txtPONO.Text)

                .AddWithValue("@itemname", row_1.Cells("ITEM_NAME").Value)
                .AddWithValue("@qty", row_1.Cells("QUANTITY").Value)
                .AddWithValue("@status", "ACTIVE")
    ...
    Sub SaveSupplier()
        ...
        Try
            cn.Open()
            cm = New MySqlCommand("INSERT into tbl_warehouse_po_8si (po_no, supplier_name, location_1, location_2, location_3, location_4, location_5, status) values(@po_no, @supplier_name, @location_1, @location_2, @location_3, @location_4, @location_5, @status)", cn)
    ...
    Sub SaveUPDATELIST()
        ...
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_purchasedrequest_select where `pr_no` like '" & txtCONTROL.Text & "' AND `status` like 'ACTIVE' ", cn)
        dr = cm.ExecuteReader
        While dr.Read
    ...
    Sub UpdateRequest()
        ...
        If txtCOUNT.Text = String.Empty Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_purchasedrequest set status=@status where id=@id", cn)
    ...
    Sub UpdateRequestSelect()
        ...
        cn.Open()
        cm = New MySqlCommand("Update tbl_8si_purchasedrequest_select set status=@status where pr_no=@pr_no and supplier_name=@supplier_name", cn)
    ...
    Sub DisplayPO()
        ...
        cn.Open()
        adp = New MySqlDataAdapter("select * from tbl_8si_purchasedorder where pr_no='" & txtREFERENCE.Text & "' AND supplier='" & txtSUPPLIER.Text & "'", cn)
        dt = New DataTable
        adp.Fill(dt)
    ...
    Private Sub frm8SI_PO_FORM_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
