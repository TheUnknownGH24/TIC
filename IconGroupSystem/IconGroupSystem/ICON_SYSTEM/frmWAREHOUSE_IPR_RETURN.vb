Public Class frmWAREHOUSE_IPR_RETURN
    ...
    Private Sub txtDEPARTMENT_TextChanged(sender As Object, e As EventArgs) Handles txtDEPARTMENT.TextChanged
        ...
        If txtDEPARTMENT.Text = "WAREHOUSE DEPARTMENT" Or txtDEPARTMENT.Text = "LOGISTICS DEPARTMENT" Then

            txtPURPOSE.Text = "STOCK TRANSFER"

        Else

            txtPURPOSE.Text = "DEPARTMENT PULL-OUT"
    ...
    Private Sub txtPURPOSE_TextChanged(sender As Object, e As EventArgs) Handles txtPURPOSE.TextChanged
        ...
        If txtPURPOSE.Text = "STOCK TRANSFER" Then

            txtORIGIN.Text = txtWAREHOUSENAME.Text

        Else

            txtORIGIN.Text = txtDEPARTMENT.Text
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_warehouse_ipr set date_required=@date_required, remarks=@remarks, date_time=@date_time, status=@status where id=@id", cn)
            ...
            MsgBox("Inventory Pull-0ut has been Successfully Update!", vbInformation + vbOKOnly, "CONFIRMATION")
    ...
    Sub DeleteInIPRList()
        ...
        cm = New MySqlCommand("DELETE FROM tbl_warehouse_ipr_item WHERE `ipr_no` like '" & txtCONTROL.Text & "'", cn)
    ...
    Sub DisplayIPR()
        ...
        cn.Open()
        adp = New MySqlDataAdapter("select * from tbl_warehouse_ipr where `transactionno` = '" & txtTRANSACTION.Text & "'", cn)
        dt = New DataTable
        adp.Fill(dt)
...
0:
    ...
    Sub SaveInIPRList()
        ...
        For Each row As DataGridViewRow In dataGridView1.Rows
            ...
            cm = New MySqlCommand("INSERT into tbl_warehouse_ipr_item (ipr_no, item_code, item_name, classification, uom, qty_requested) values(@ipr_no, @item_code, @item_name, @classification, @uom, @qty_requested)", cn)

            With cm.Parameters

                .AddWithValue("@ipr_no", txtCONTROL.Text)
    ...
    Sub LoadIPRList()
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_warehouse_ipr where `STATUS` = 'RETURNED' ", cn)
            dr = cm.ExecuteReader

            While dr.Read
                i += 1
                DataGridView3.Rows.Add(i, dr.Item("id").ToString, dr.Item("ipr_no").ToString, dr.Item("date").ToString, dr.Item("transactionno").ToString, dr.Item("date_required").ToString, dr.Item("requestor").ToString, dr.Item("department").ToString, dr.Item("purpose").ToString, dr.Item("destination").ToString, dr.Item("origin").ToString, dr.Item("remarks").ToString, dr.Item("manager_name").ToString, dr.Item("manager_approval").ToString, dr.Item("manager_comment").ToString, dr.Item("final_name").ToString, dr.Item("final_approval").ToString, dr.Item("final_comment").ToString, dr.Item("date_time").ToString, dr.Item("status").ToString)
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_warehouse_ipr where `STATUS` = 'RETURNED' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%' ", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_warehouse_ipr where ipr_no like '%" & TextBox1.Text & "%' AND `STATUS` = 'RETURNED' ", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_warehouse_ipr where ipr_no like '%" & TextBox1.Text & "%' AND `STATUS` = 'RETURNED' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%'", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub DataGridView3_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView3.CellContentClick
        ...
        If colname = "MANAGER_APPROVAL" Then
            cn.Open()
            cm = New MySqlCommand("select * from tbl_warehouse_ipr where id like '" & DataGridView3.Rows(e.RowIndex).Cells(1).Value.ToString & "'", cn)
            dr = cm.ExecuteReader
            While dr.Read
                ...
                txtDESTINATION.Text = DataGridView3.Rows(e.RowIndex).Cells(9).Value.ToString
                txtORIGIN.Text = DataGridView3.Rows(e.RowIndex).Cells(10).Value.ToString

                txtREMARKS.Text = DataGridView3.Rows(e.RowIndex).Cells(11).Value.ToString

                txtMANAGERCOMMENT.Text = DataGridView3.Rows(e.RowIndex).Cells(12).Value.ToString & " - " & DataGridView3.Rows(e.RowIndex).Cells(14).Value.ToString
                txtGMCOMMENT.Text = DataGridView3.Rows(e.RowIndex).Cells(15).Value.ToString & " - " & DataGridView3.Rows(e.RowIndex).Cells(17).Value.ToString

                txtAPPROVAL_1.Text = DataGridView3.Rows(e.RowIndex).Cells(13).Value.ToString
                txtAPPROVAL_2.Text = DataGridView3.Rows(e.RowIndex).Cells(16).Value.ToString
