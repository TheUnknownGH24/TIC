...
Public Class frm8SI_PO_GM_FORM
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        Else
            ...
            If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
                ...
                cn.Open()
                cm = New MySqlCommand("Update tbl_8si_purchasedorder set required_date=@required_date, GM_APPROVAL=@GM_APPROVAL, gm_comment=@gm_comment where id=@id", cn)
                With cm.Parameters
    ...
    Sub DeleteInPOList()
        ...
        cm = New MySqlCommand("DELETE FROM tbl_8si_purchasedorderlist WHERE `po_no` like '" & txtPONO.Text & "'", cn)
    ...
    Private Sub btnPRINT_Click(sender As Object, e As EventArgs) Handles btnPRINT.Click
        ...
        Else
            ...
            If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
                ...
                cn.Open()
                cm = New MySqlCommand("Update tbl_8si_purchasedorder set required_date=@required_date, GM_APPROVAL=@GM_APPROVAL, gm_comment=@gm_comment, status=@status where id=@id", cn)
                With cm.Parameters
                    ...
                    .AddWithValue("@gm_comment", txtMANAGERCOMMENT.Text & " - " & DateTimePicker1.Value)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles btnRETURN.Click
        ...
        Else
            ...
            If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
                ...
                cn.Open()
                cm = New MySqlCommand("Update tbl_8si_purchasedorder set required_date=@required_date, MANAGER_APPROVAL=@MANAGER_APPROVAL, manager_comment=@manager_comment, GM_APPROVAL=@GM_APPROVAL, gm_comment=@gm_comment, status=@status where id=@id", cn)
                With cm.Parameters
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_purchasedorder where pr_no like '%" & TextBox1.Text & "%' AND `MANAGER_APPROVAL` = 'APPROVED' AND `GM_APPROVAL` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadPOList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_purchasedorder where `MANAGER_APPROVAL` = 'APPROVED' AND `GM_APPROVAL` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader

        While dr.Read
            i += 1
            DataGridView2.Rows.Add(i, dr.Item("id").ToString, dr.Item("po_no").ToString, dr.Item("pr_no").ToString, dr.Item("employee").ToString, dr.Item("department").ToString, dr.Item("date").ToString, dr.Item("shipping_method").ToString, dr.Item("payment_terms").ToString, dr.Item("required_date").ToString, dr.Item("delivery_address").ToString, dr.Item("supplier").ToString, dr.Item("manager_comment").ToString)
    ...
    Private Sub frm8SI_PO_GM_FORM_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
