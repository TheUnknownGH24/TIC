...
Public Class frmTIC_PR_GM_FORM_FINAL
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        Else
            ...
            If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
                ...
                cn.Open()
                cm = New MySqlCommand("Update tbl_tic_purchasedrequest set purpose=@purpose, GM_APPROVAL=@GM_APPROVAL, gm_comment=@gm_comment where id=@id", cn)
                With cm.Parameters
                    .AddWithValue("@id", txtID.Text)
                    .AddWithValue("@purpose", txtREMARKS.Text)
                    .AddWithValue("@GM_APPROVAL", "APPROVED")
                    .AddWithValue("@gm_comment", txtGMCOMMENT.Text & " - " & DateTimePicker1.Value)
                End With
                cm.ExecuteNonQuery()
                cn.Close()
                'DeleteInPRListAll()
    ...
    Sub UpdateRequest()
        ...
        cn.Open()
        cm = New MySqlCommand("Update tbl_tic_requestslip set status=@status where id=@id", cn)
    ...
    Sub DeleteInPRListAll()
        ...
        cm = New MySqlCommand("DELETE FROM tbl_tic_purchasedrequest_all WHERE `pr_no` like '%" & txtCONTROL.Text & "%'", cn)
    ...
    Sub DeleteInPRList()
        ...
        cm = New MySqlCommand("DELETE FROM tbl_tic_purchasedrequestlist WHERE `pr_no` like '%" & txtCONTROL.Text & "%'", cn)
    ...
    Sub DisplayPR()
        ...
        cn.Open()
        adp = New MySqlDataAdapter("select * from tbl_tic_purchasedrequest where transaction_no = '" & txtTRANSACTION.Text & "'", cn)
        dt = New DataTable
        adp.Fill(dt)
    ...
    Sub SaveInPRList_1()
        ...
        For Each row_1 As DataGridViewRow In dataGridView1.Rows
            ...
            cm = New MySqlCommand("INSERT into tbl_tic_purchasedrequestlist (pr_no, transaction_no, supplier_name, quantity, unit, item_name, unit_price, vat_amount, grand_total) values(@pr_no, @transaction_no, @supplier_name, @quantity, @unit, @item_name, @unit_price, @vat_amount, @grand_total)", cn)
    ...
    Private Sub btnPRINT_Click(sender As Object, e As EventArgs) Handles btnPRINT.Click
        ...
        Else
            ...
            If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
                ...
                cn.Open()
                cm = New MySqlCommand("Update tbl_tic_purchasedrequest set purpose=@purpose, GM_APPROVAL=@GM_APPROVAL, gm_comment=@gm_comment, status=@status where id=@id", cn)
                With cm.Parameters
                    .AddWithValue("@id", txtID.Text)
                    .AddWithValue("@purpose", txtREMARKS.Text)
                ...
                MsgBox("Purchased Request has been Successfully Disapproved!", vbInformation + vbOKOnly, "CONFIRMATION")
    ...
    Private Sub btnRETURN_Click(sender As Object, e As EventArgs) Handles btnRETURN.Click
        ...
        Else
            ...
            If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
                ...
                cn.Open()
                cm = New MySqlCommand("Update tbl_tic_purchasedrequest set purpose=@purpose, MANAGER_APPROVAL=@MANAGER_APPROVAL, manager_comment=@manager_comment, GM_APPROVAL=@GM_APPROVAL, gm_comment=@gm_comment, status=@status where id=@id", cn)
                With cm.Parameters
                    .AddWithValue("@id", txtID.Text)
                    .AddWithValue("@purpose", txtREMARKS.Text)
                ...
                MsgBox("Purchased Request has been Successfully Return!", vbInformation + vbOKOnly, "CONFIRMATION")
    ...
    Private Sub txtSEARCHNAME_TextChanged(sender As Object, e As EventArgs) Handles txtSEARCHNAME.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_purchasedrequest where pr_no like '%" & txtSEARCHNAME.Text & "%' AND `MANAGER_APPROVAL` = 'APPROVED' AND `GM_APPROVAL` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE'", cn)
        dr = cm.ExecuteReader

        While dr.Read
            i += 1
            DataGridView5.Rows.Add(i, dr.Item("id").ToString, dr.Item("pr_no").ToString, dr.Item("transaction_no").ToString, dr.Item("employee").ToString, dr.Item("department").ToString, dr.Item("date").ToString, dr.Item("purpose").ToString, dr.Item("supplier_1").ToString, dr.Item("supplier_2").ToString, dr.Item("supplier_3").ToString, dr.Item("manager_comment").ToString)
    ...
    Private Sub DataGridView5_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView5.CellContentClick
        ...
        If colname = "MANAGER_APPROVAL" Then
            ...
            While dr.Read
                ...
                txtCOMMENT.Text = DataGridView5.Rows(e.RowIndex).Cells(11).Value.ToString
    ...
    Sub LoadPRList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_purchasedrequest where `MANAGER_APPROVAL` = 'APPROVED' and `GM_APPROVAL` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader
    ...
    Sub Add_Approval_Button()
        ...
        End With
        DataGridView5.Columns.Insert(12, colApproval)
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        FRMTIC_PR_GM_FORM_VIEW.txtSUPPLIER_1.Text = txtSUPPLIER_1.Text
        FRMTIC_PR_GM_FORM_VIEW.txtSUPPLIER_2.Text = txtSUPPLIER_2.Text
        FRMTIC_PR_GM_FORM_VIEW.txtSUPPLIER_3.Text = txtSUPPLIER_3.Text

        FRMTIC_PR_GM_FORM_VIEW.txtPRNO_1.Text = txtCONTROL.Text
        FRMTIC_PR_GM_FORM_VIEW.txtPRNO_2.Text = txtCONTROL.Text
        FRMTIC_PR_GM_FORM_VIEW.txtPRNO_3.Text = txtCONTROL.Text

        FRMTIC_PR_GM_FORM_VIEW.LoadPURCHASEDList_supplier1_FINAL()
        FRMTIC_PR_GM_FORM_VIEW.LoadPURCHASEDList_supplier2_FINAL()
        FRMTIC_PR_GM_FORM_VIEW.LoadPURCHASEDList_supplier3_FINAL()

        FRMTIC_PR_GM_FORM_VIEW.Supplier_sum()

        FRMTIC_PR_GM_FORM_VIEW.StartPosition = FormStartPosition.CenterScreen
        FRMTIC_PR_GM_FORM_VIEW.Show()


    End Sub

    Private Sub frmTIC_PR_GM_FORM_FINAL_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
