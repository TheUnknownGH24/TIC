Public Class frmWAREHOUSE_DR_MIR
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_warehouse_mir where `mir_no` like '%" & TextBox1.Text & "%' AND `manager_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'ACTIVE' OR `STATUS` = 'PARTIALLY DELIVERED' ", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_warehouse_mir where `manager_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'ACTIVE' OR `STATUS` = 'PARTIALLY DELIVERED' ", cn)
        dr = cm.ExecuteReader
    ...
    Sub DR_UPDATE()
        ...
        If txtSTATUS.Text = "PARTIALLY DELIVERED" Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_warehouse_mir set status=@status where mir_no=@mir_no", cn)
