Public Class frmWAREHOUSE_MIR_MANAGER
    ...
    Sub LoadIPRList()
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_warehouse_mir where `manager_approval` = 'FOR APPROVAL' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE' ", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_warehouse_mir where `manager_approval` = 'FOR APPROVAL' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%' ", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_warehouse_mir where mir_no like '%" & TextBox1.Text & "%' AND `manager_approval` = 'FOR APPROVAL' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE' ", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_warehouse_mir where mir_no like '%" & TextBox1.Text & "%' AND `manager_approval` = 'FOR APPROVAL' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%'", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        Try
            ...
            If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
                ...
                cn.Open()
                cm = New MySqlCommand("Update tbl_warehouse_mir set manager_name=@manager_name, manager_approval=@manager_approval, manager_comment=@manager_comment, status=@status where id=@id", cn)
