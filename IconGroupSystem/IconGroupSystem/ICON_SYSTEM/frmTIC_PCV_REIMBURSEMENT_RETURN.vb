Public Class frmTIC_PCV_REIMBURSEMENT_RETURN
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_pcv_reimbursement where `pcv_no` like '%" & TextBox3.Text & "%' AND `STATUS` = 'RETURNED'", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_pcv_reimbursement where `pcv_no` like '%" & TextBox3.Text & "%' AND `STATUS` = 'RETURNED' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%'", cn)
            dr = cm.ExecuteReader
    ...
    Sub LoadREIMList()
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_pcv_reimbursement where `STATUS` = 'RETURNED' ", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_pcv_reimbursement where `STATUS` = 'RETURNED' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%'", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub DataGridView2_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView2.CellContentClick
        ...
        If colname = "MANAGER_APPROVAL" Then
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_pcv_reimbursement where id like '" & DataGridView2.Rows(e.RowIndex).Cells(1).Value.ToString & "'", cn)
            dr = cm.ExecuteReader
            While dr.Read
    ...
    Sub LoadREIMITEMLIST()
        ...
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_pcv_reimbursement_list where `pcv_no` like '%" & txtCONTROL.Text & "%' ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_pcv_reimbursement set total_liquidation=@total_liquidation, purpose=@purpose, date_time=@date_time, status=@status where id=@id", cn)
            ...
            MsgBox("PCV Reimbursement has been Successfully Update!", vbInformation + vbOKOnly, "CONFIRMATION")
    ...
    Sub DeleteInREQUESTList()
        ...
        cm = New MySqlCommand("DELETE FROM tbl_tic_pcv_reimbursement_list WHERE `pcv_no` like '" & txtCONTROL.Text & "'", cn)
    ...
    Sub SaveInREIMList()
        ...
        For Each row As DataGridViewRow In dataGridView1.Rows
            ...
            cm = New MySqlCommand("INSERT into tbl_tic_pcv_reimbursement_list (pcv_no, expense_categories, date_of_receipt, supplier_name, receipt_no, receipt_amount, remarks) values(@pcv_no, @expense_categories, @date_of_receipt, @supplier_name, @receipt_no, @receipt_amount, @remarks)", cn)
            With cm.Parameters
    ...
    Private Sub frmTIC_PCV_REIMBURSEMENT_RETURN_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
