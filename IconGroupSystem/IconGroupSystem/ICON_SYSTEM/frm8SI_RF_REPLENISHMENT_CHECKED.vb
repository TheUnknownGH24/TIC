Public Class frm8SI_RF_REPLENISHMENT_CHECKED
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_rf_replenishment where `rf_no` like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `reviewed_approval` = 'FOR APPROVAL' AND `finance_approval` = 'FOR APPROVAL' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadRFList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_rf_replenishment where `endorsed_approval` = 'APPROVED' AND `reviewed_approval` = 'FOR APPROVAL' AND `finance_approval` = 'FOR APPROVAL' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_rf_replenishment set reviewed_approval=@reviewed_approval, reviewed_comment=@reviewed_comment, status=@status where id=@id", cn)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_rf_replenishment set endorsed_approval=@endorsed_approval, reviewed_approval=@reviewed_approval, reviewed_comment=@reviewed_comment, status=@status where id=@id", cn)
    ...
    Private Sub frm8SI_RF_REPLENISHMENT_CHECKED_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
