Public Class frmVIEW_TIC_CA_LIQUIDATION_FINANCE
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_cashadvance_liquidation where `cano_liquidation` like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'FOR APPROVAL' OR `finance_approval` = 'APPROVED' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ON-GOING LIQUIDATION'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadCAList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_cashadvance_liquidation where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'FOR APPROVAL' OR `finance_approval` = 'APPROVED' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ON-GOING LIQUIDATION' ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub Button1_Click_1(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_cashadvance_liquidation set finance_approval=@finance_approval, finance_comment=@finance_comment where id=@id", cn)
