...
Public Class frmCOO_PR_APPROVAL_LIST
    ...
    Sub LoadPRList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tblpurchasedrequest where `MANAGER_APPROVAL` = 'APPROVED' AND `GM_APPROVAL` = 'APPROVED' AND `COO_APPROVAL` = 'FOR APPROVAL'", cn)
        dr = cm.ExecuteReader
        While dr.Read
            i += 1
    ...
    Private Sub txtSEARCHNAME_TextChanged(sender As Object, e As EventArgs) Handles txtSEARCHNAME.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tblpurchasedrequest where pr_no like '%" & txtSEARCHNAME.Text & "%' AND `MANAGER_APPROVAL` = 'APPROVED' AND `GM_APPROVAL` = 'APPROVED' AND `COO_APPROVAL` = 'FOR APPROVAL'", cn)
        dr = cm.ExecuteReader
