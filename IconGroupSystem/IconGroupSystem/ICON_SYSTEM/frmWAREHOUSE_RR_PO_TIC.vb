Public Class frmWAREHOUSE_RR_PO_TIC
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_warehouse_po_tic where `po_no` like '%" & TextBox1.Text & "%' AND `STATUS` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader

        While dr.Read
            i += 1
            DataGridView2.Rows.Add(i, dr.Item("id").ToString, dr.Item("po_no").ToString, dr.Item("supplier_name").ToString)
    ...
    Sub LoadPOList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_warehouse_po_tic where `STATUS` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader
    ...
    Sub Add_Approval_Button()
        ...
        End With
        DataGridView2.Columns.Insert(4, colApproval)
    ...
    Private Sub DataGridView2_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView2.CellContentClick
        ...
        If colname = "MANAGER_APPROVAL" Then
            cn.Open()
            cm = New MySqlCommand("select * from tbl_warehouse_po_tic where id like '" & DataGridView2.Rows(e.RowIndex).Cells(1).Value.ToString & "'", cn)
            dr = cm.ExecuteReader
            While dr.Read

                txtPONO.Text = DataGridView2.Rows(e.RowIndex).Cells(2).Value.ToString
                txtSUPPLIERNAME.Text = DataGridView2.Rows(e.RowIndex).Cells(3).Value.ToString

                txtPURPOSE.Text = "PURCHASES"
    ...
    Sub LoadItemList()
        ...
        If txtRR_REFERENCE.Text = String.Empty Or txtRR_REFERENCE.Text = "" Then
            ...
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_purchasedorderlist where `po_no` like '" & txtPONO.Text & "' and supplier_name like '" & txtSUPPLIERNAME.Text & "'", cn)
            dr = cm.ExecuteReader
            While dr.Read
