Public Class frm8SI_PCV_LIQUIDATION_FINANCE
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
            ...
            Dim i As Integer
            cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_pettycash_liquidation where `pcv_no` like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'FOR APPROVAL' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ON-GOING LIQUIDATION'", cn)
            dr = cm.ExecuteReader
    ...
    Sub LoadPCVList()
            ...
            Dim i As Integer
            cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_pettycash_liquidation where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'FOR APPROVAL' AND `final_approval` = 'FOR APPROVAL' AND `STATUS` = 'ON-GOING LIQUIDATION' ", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_pettycash_liquidation set finance_approval=@finance_approval, finance_comment=@finance_comment, status=@status where id=@id", cn)
    ...
    Private Sub frm8SI_PCV_LIQUIDATION_FINANCE_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
