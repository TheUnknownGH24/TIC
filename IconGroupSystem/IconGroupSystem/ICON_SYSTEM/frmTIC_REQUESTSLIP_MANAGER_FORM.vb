Public Class frmTIC_REQUESTSLIP_MANAGER_FORM
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnUPDATE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_requestslip set date_needed=@date_needed, manager_approval=@manager_approval, manager_comment=@manager_comment where id=@id", cn)
    ...
    Private Sub btnPRINT_Click(sender As Object, e As EventArgs) Handles btnPRINT.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_requestslip set date_needed=@date_needed, manager_approval=@manager_approval, manager_comment=@manager_comment, status=@status where id=@id", cn)
    ...
    Private Sub txtSEARCHNAME_TextChanged(sender As Object, e As EventArgs) Handles txtSEARCHNAME.TextChanged
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_requestslip where `controlno` like '%" & txtSEARCHNAME.Text & "%' AND `manager_approval` = 'FOR APPROVAL' AND `status` = 'ACTIVE'", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_requestslip where `controlno` like '%" & txtSEARCHNAME.Text & "%' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%' AND `manager_approval` = 'FOR APPROVAL' AND `status` = 'ACTIVE'", cn)
            dr = cm.ExecuteReader
    ...
    Sub LoaDAPPROVALList()
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_requestslip where `manager_approval` = 'FOR APPROVAL' AND `status` = 'ACTIVE'", cn)
            dr = cm.ExecuteReader
            While dr.Read
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_requestslip where `manager_approval` = 'FOR APPROVAL' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%' AND `status` = 'ACTIVE'", cn)
            dr = cm.ExecuteReader
            While dr.Read
    ...
    Private Sub frmTIC_REQUESTSLIP_MANAGER_FORM_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
