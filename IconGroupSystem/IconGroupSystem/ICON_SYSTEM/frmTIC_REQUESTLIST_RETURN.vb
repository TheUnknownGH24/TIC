...
Public Class frmTIC_REQUESTLIST_RETURN
    ...
    Private Sub txtSEARCHNAME_TextChanged(sender As Object, e As EventArgs) Handles txtSEARCHNAME.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_requestslip where `controlno` like '%" & txtSEARCHNAME.Text & "%' AND `gm_approval` = 'FOR APPROVAL' AND `status` = 'RETURN' ", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadREQUESTList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_requestslip where `gm_approval` = 'FOR APPROVAL' AND `status` = 'RETURN' ", cn)
        dr = cm.ExecuteReader
        While dr.Read
            i += 1
            dataGridView1.Rows.Add(i, dr.Item("id").ToString, dr.Item("controlno").ToString, dr.Item("transaction_no").ToString, dr.Item("date_now").ToString, dr.Item("employee_name").ToString, dr.Item("department").ToString, dr.Item("date_needed").ToString, dr.Item("date_time").ToString, dr.Item("gm_comment").ToString)
    ...
    Private Sub dataGridView1_CellContentClick_1(sender As Object, e As DataGridViewCellEventArgs) Handles dataGridView1.CellContentClick
        ...
        If colname = "colUPDATE" Then
            With frmTIC_REQUESTSLIP_RETURN
                cn.Open()
    ...
    Private Sub frmTIC_REQUESTLIST_RETURN_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
