Public Class frmWAREHOUSE_DR_IPR
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_warehouse_ipr where `ipr_no` like '%" & TextBox1.Text & "%' AND `manager_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'ACTIVE' OR `STATUS` = 'PARTIALLY DELIVERED' ", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_warehouse_ipr where `manager_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'ACTIVE' OR `STATUS` = 'PARTIALLY DELIVERED' ", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadItemList()
        ...
        cn.Open()
        cm = New MySqlCommand("select * from tbl_warehouse_ipr_item where `ipr_no` like '" & txtPONO.Text & "' ", cn)
        dr = cm.ExecuteReader
        While dr.Read
    ...
    Sub DR_UPDATE()
        ...
        If txtSTATUS.Text = "PARTIALLY DELIVERED" Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_warehouse_ipr set status=@status where ipr_no=@ipr_no", cn)
