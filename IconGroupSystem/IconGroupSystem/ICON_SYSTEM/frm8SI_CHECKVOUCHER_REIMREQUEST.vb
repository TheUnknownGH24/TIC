Public Class frm8SI_CHECKVOUCHER_REIMREQUEST
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_reimbursement where `reimbursementno` like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'APPROVED BY MS.VTY'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadREIMList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_reimbursement where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'APPROVED BY MS.VTY'", cn)
        dr = cm.ExecuteReader
    ...
    Sub CV_UPDATE()
        ...
        cn.Open()
        cm = New MySqlCommand("Update tbl_8si_reimbursement set status=@status where id=@id", cn)
    ...
    Private Sub frm8SI_CHECKVOUCHER_REIMREQUEST_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
