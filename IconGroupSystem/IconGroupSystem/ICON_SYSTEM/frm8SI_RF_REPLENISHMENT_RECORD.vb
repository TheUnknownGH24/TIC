Public Class frm8SI_RF_REPLENISHMENT_RECORD
    ...
    Sub LoadRFList()
        ...
        ElseIf txtDEP.Text = "DRS - ADMIN" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_8si_rf_replenishment where `department` like 'DIRECT SELLING - OPERATIONS' OR `department` like 'DIRECT SELLING - OPERATIONS - (CENTRAL AND NORTH LUZON)' OR `department` like 'DIRECT SELLING - OPERATIONS - (MINDANAO)' OR `department` like 'DIRECT SELLING - OPERATIONS - (NCR AND SOUTH LUZON)' OR `department` like 'DIRECT SELLING - OPERATIONS - (VISAYAS)' ", cn)
            dr = cm.ExecuteReader
