Public Class frmVIEW_8SI_CHECKVOUCHER_FINANCE
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_checkvoucher where `cv_no` like '%" & TextBox3.Text & "%' AND `manager_approval` = 'FOR APPROVAL' OR `manager_approval` = 'APPROVED' AND `gm_approval` = 'FOR APPROVAL' AND `coo_approval` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadCVList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_checkvoucher where `manager_approval` = 'FOR APPROVAL' OR `manager_approval` = 'APPROVED' AND `gm_approval` = 'FOR APPROVAL' AND `coo_approval` = 'FOR APPROVAL' AND `STATUS` = 'ACTIVE'", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub btnAPPROVED_Click(sender As Object, e As EventArgs) Handles btnAPPROVED.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_checkvoucher set manager_approval=@manager_approval, manager_comment=@manager_comment, gm_approval=@gm_approval, status=@status where id=@id", cn)
            ...
            With cm.Parameters
                ...
                .AddWithValue("@manager_comment", txtFINANCE.Text & "-" & DateTimePicker1.Value)
    ...
    Private Sub btnDISAPPROVED_Click(sender As Object, e As EventArgs) Handles btnDISAPPROVED.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_checkvoucher set manager_approval=@manager_approval, manager_comment=@manager_comment, status=@status where id=@id", cn)
    ...
    Private Sub frm8SI_CHECKVOUCHER_FINANCE_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
