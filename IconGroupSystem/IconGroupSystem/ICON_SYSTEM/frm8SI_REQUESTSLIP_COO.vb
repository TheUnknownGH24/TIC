Public Class frm8SI_REQUESTSLIP_COO
    ...
    Private Sub btnPRINT_Click(sender As Object, e As EventArgs) Handles btnPRINT.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_8si_requestslip set date_needed=@date_needed, gm_approval=@gm_approval, gm_comment=@gm_comment, status=@status where id=@id", cn)
    ...
    Private Sub txtSEARCHNAME_TextChanged(sender As Object, e As EventArgs) Handles txtSEARCHNAME.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_requestslip where `controlno` like '%" & txtSEARCHNAME.Text & "%' AND `manager_approval` = 'APPROVED' AND `gm_approval` = 'FOR APPROVAL' AND `status` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadAPPROVALLIST()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_8si_requestslip where `manager_approval` = 'APPROVED' AND `gm_approval` = 'APPROVED' AND `status` = 'ACTIVE'", cn)
        dr = cm.ExecuteReader
        While dr.Read
            i += 1
            DataGridView2.Rows.Add(i, dr.Item("id").ToString, dr.Item("controlno").ToString, dr.Item("transaction_no").ToString, dr.Item("date_now").ToString, dr.Item("employee_name").ToString, dr.Item("department").ToString, dr.Item("date_needed").ToString, dr.Item("date_time").ToString, dr.Item("manager_comment").ToString)
    ...
    Private Sub frm8SI_REQUESTSLIP_GM_FORM_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
