Public Class frmTIC_REIMBURSEMENT_RETURN
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_reimbursement where `reimbursementno` like '%" & TextBox3.Text & "%' AND `STATUS` = 'RETURNED'", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtDEPARTMENTDATA.Text = "DIRECT SELLING - OPERATIONS" Or txtDEPARTMENTDATA.Text = "DIRECT SELLING - BUSINESS DEV" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_reimbursement where employee like '%" & txtNAME.Text & "%' AND `reimbursementno` like '%" & TextBox3.Text & "%' AND `STATUS` = 'RETURNED'", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_reimbursement where `reimbursementno` like '%" & TextBox3.Text & "%' AND `STATUS` = 'RETURNED' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%'", cn)
            dr = cm.ExecuteReader
    ...
    Sub LoadREIMList()
        ...
        If txtTYPE.Text = "ADMINISTRATOR" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_reimbursement where `STATUS` = 'RETURNED' ", cn)
            dr = cm.ExecuteReader
        ...
        ElseIf txtTYPE.Text = "USER" Then
            ...
            Dim i As Integer
            cn.Open()
            cm = New MySqlCommand("select * from tbl_tic_reimbursement where `STATUS` = 'RETURNED' AND `department` like '%" & txtDEPARTMENTDATA.Text & "%'", cn)
            dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_reimbursement set purpose=@purpose, total_liquidation=@total_liquidation, date_time=@date_time, status=@status where id=@id", cn)
    ...
    Sub DeleteInREQUESTList()
        ...
        cm = New MySqlCommand("DELETE FROM tbl_tic_reimbursement_list WHERE `reimbursementno` like '" & txtCONTROL.Text & "'", cn)
    ...
    Sub SaveInREIMList()
        ...
        For Each row As DataGridViewRow In dataGridView1.Rows
            ...
            cm = New MySqlCommand("INSERT into tbl_tic_reimbursement_list (reimbursementno, expense_categories, date_of_receipt, supplier_name, receipt_no, receipt_amount, remarks) values(@reimbursementno, @expense_categories, @date_of_receipt, @supplier_name, @receipt_no, @receipt_amount, @remarks)", cn)
            With cm.Parameters
    ...
    Private Sub frmTIC_REIMBURSEMENT_RETURN_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
