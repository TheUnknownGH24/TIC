Public Class frm8SI_CASHADVANCE
    ...
    Private Sub btnSAVE_Click(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("INSERT into tbl_8si_cashadvance (transaction_no, date, employee, department, depcode, date_needed, purpose, total_amount, endorsed_approval, endorsed_comment, finance_approval, finance_comment, final_approval, final_comment, date_time, status) values(@transaction_no, @date, @employee, @department, @depcode, @date_needed, @purpose, @total_amount, @endorsed_approval, @endorsed_comment, @finance_approval, @finance_comment, @final_approval, @final_comment, @date_time, @status)", cn)
            With cm.Parameters
                .AddWithValue("@transaction_no", txtTRANSACTION.Text)
                .AddWithValue("@date", Format(CDate(txtDATE.Text), myFormat))
    ...
    Sub DisplayCANO()
        ...
        cn.Open()
        adp = New MySqlDataAdapter("select * from tbl_8si_cashadvance where transaction_no = '" & txtTRANSACTION.Text & "'", cn)
        dt = New DataTable
        adp.Fill(dt)
    ...
    Private Sub frm8SI_CASHADVANCE_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
