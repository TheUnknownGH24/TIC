Public Class frmTIC_CA_LIQUIDATION_COO
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_cashadvance_liquidation where `cano_liquidation` like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'ON-GOING LIQUIDATION'", cn)
        dr = cm.ExecuteReader
    ...
    Sub LoadCAList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_cashadvance_liquidation where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'APPROVED' AND `STATUS` = 'ON-GOING LIQUIDATION' ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click_1(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_cashadvance_liquidation set final_comment=@final_comment, status=@status where id=@id", cn)

            With cm.Parameters

                .AddWithValue("@id", txtID.Text)

                .AddWithValue("@final_comment", txtFINALCOMMENT.Text & "-" & DateTimePicker1.Value & " / " & txtCOO.Text & "-" & DateTimePicker1.Value)

                .AddWithValue("@status", "APPROVED BY MS.VTY")
    ...
    Private Sub Button1_Click_1(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_cashadvance_liquidation set final_approval=@final_approval, final_comment=@final_comment, status=@status where id=@id", cn)
            ...
            With cm.Parameters
                ...
                .AddWithValue("@final_comment", txtFINALCOMMENT.Text & "-" & DateTimePicker1.Value)
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_cashadvance_liquidation set endorsed_approval=@endorsed_approval, endorsed_comment=@endorsed_comment, finance_approval=@finance_approval, finance_comment=@finance_comment, final_approval=@final_approval, final_comment=@final_comment, status=@status where id=@id", cn)
            ...
            MsgBox("CA LIQUIDATION has been Successfully Return!", vbInformation + vbOKOnly, "CONFIRMATION")
    ...
    Private Sub frm8SI_CA_LIQUIDATION_GM_COO_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
