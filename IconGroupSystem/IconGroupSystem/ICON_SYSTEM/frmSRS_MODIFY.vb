...
Public Class frmSRS_MODIFY
    ...
    Sub LoadSRSList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tblsrs_request where `requestor` like '%" & frm1HOME.txtHOMENAME.Text & "%' and `manager_approval` like 'FOR APPROVAL' ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub txtSEARCHNAME_TextChanged(sender As Object, e As EventArgs) Handles txtSEARCHNAME.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tblsrs_request where `srs_no` like '%" & txtSEARCHNAME.Text & "%' AND `MANAGER_APPROVAL` = 'FOR APPROVAL' AND `requestor` like '%" & frm1HOME.txtHOMENAME.Text & "%'", cn)
        dr = cm.ExecuteReader
