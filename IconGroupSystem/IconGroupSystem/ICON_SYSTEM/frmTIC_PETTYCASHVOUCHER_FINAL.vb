Public Class frmTIC_PETTYCASHVOUCHER_FINAL
    ...
    Private Sub TextBox3_TextChanged(sender As Object, e As EventArgs) Handles TextBox3.TextChanged
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_pettycash where `pcv_no` like '%" & TextBox3.Text & "%' AND `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'FOR RELEASE' AND `STATUS` = 'ACTIVE'", cn)
        dr = cm.ExecuteReader
        While dr.Read
            i += 1
    ...
    Sub LoadPCVList()
        ...
        Dim i As Integer
        cn.Open()
        cm = New MySqlCommand("select * from tbl_tic_pettycash where `endorsed_approval` = 'APPROVED' AND `finance_approval` = 'APPROVED' AND `final_approval` = 'FOR RELEASE' AND `STATUS` = 'ACTIVE' ", cn)
        dr = cm.ExecuteReader
    ...
    Private Sub btnSAVE_Click_1(sender As Object, e As EventArgs) Handles btnSAVE.Click
        ...
        If MsgBox("Do you want to Proceed ?", vbYesNo + vbQuestion, "CONFIRMATION") = vbYes Then
            ...
            cn.Open()
            cm = New MySqlCommand("Update tbl_tic_pettycash set final_approval=@final_approval, final_comment=@final_comment, status=@status where id=@id", cn)
    ...
    Private Sub frmTIC_PETTYCASHVOUCHER_FINAL_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
